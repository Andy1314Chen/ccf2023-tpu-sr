...
def main():
    ...
    if args.predict_method == "dataset":
        ...
        # # single thread
        # history_size = 0
        # for _idx, _input_size in tqdm(enumerate(input_size_list)):
        #     h, w, ofs = _input_size
        #     ofs -= history_size

        #     tile_num = math.ceil(h / args.tile_y) * math.ceil(w / args.tile_x)
        #     history_size += tile_num
        #     _output_tile_list = copy.copy(output_tile_list[: tile_num])
        #     output_tile_list = output_tile_list[tile_num:]
        #     gc.collect()

        #     res = multi_calc_niqe(_idx, _input_size, _output_tile_list)
        #     result.append(res)

        # # multi thread
        # import threading

        # size_per_thread = int(len(input_size_list) / 2)

        # start_idx = 0
        # thread1 = threading.Thread(target=thread_calc_niqe,
        #                            args=(start_idx, input_size_list[:size_per_thread]))
        # start_idx = size_per_thread
        # thread2 = threading.Thread(target=thread_calc_niqe,
        #                            args=(start_idx, input_size_list[size_per_thread:]))
        # # 启动线程
        # thread1.start()
        # thread2.start()
        ...
        # for _idx in tqdm(range(len(input_size_list))):
        #     _input_size = input_size_list[_idx]
        #     h, w, ofs = _input_size

        #     tile_num = math.ceil(h / args.tile_y) * math.ceil(w / args.tile_x)
        #     _output_tile_list = copy.copy(
        #         output_tile_list[ofs: ofs + tile_num])
        #     future = thread_pool.submit(
        #         multi_calc_niqe, _idx, _input_size, _output_tile_list)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--predict_method", type=str, default="old",
                        help="predict method (old/new/batch/dataset)")
    parser.add_argument("-b", "--batch_size", type=int, default=16,
                        help="inference batch size")
    parser.add_argument("-x", "--tile_x", type=int, default=56,
                        help="x tile size (width)")
    parser.add_argument("-y", "--tile_y", type=int, default=56,
                        help="y tile size (height)")
    parser.add_argument("-t", "--tile_pad", type=int, default=4,
                        help="tile padding")
